...
export default function ProjectItem({ image, name, category, id }: ProjectTypes) {
  return (
    <div className="w-full md:w-1/2 px-4 rounded-2xl md:mb-0 mb-[40px] mt-14 text-center flex justify-center">
      <div className="item md:w-[400px] flex flex-col items-center justify-end whitespace-normal bg-white rounded-2xl relative ">
        <div className="relative group overflow-hidden">
          <div className="absolute top-1/2 left-1/2 -translate-y-[250px] group-hover:-translate-y-1/2 -translate-x-1/2 z-20 transition ease-in-out duration-500">
            <img src="/images/ic_play.svg" className="lg:w-20 w-12" alt="" />
          </div>
          <img src={image[0]} alt="console" className="rounded-2xl relative w-[700px] h-[260px] object-cover" />
          <div className="absolute top-0 right-0 p-2 rounded-bl-2xl bg-white rounded-tr-2xl">
            <div className="flex items-center">
              ...
              <h1 className="text-kBlackColor ml-2 font-medium text-[24px] ">4.8</h1>
            </div>
          </div>
        </div>
        <div>
          <p className="mt-[20px] relative text-xl text-kBlackColor mb-2 text-center font-medium line-clamp1" style={{ wordWrap: 'break-word' }}>
            {name}
          </p>
        </div>
        <p className="text-xl text-kGreyColor1 mb-[40px] text-center">
          {category}
        </p>
